...
clf = nb.NaiveBayesTextClassifier()  # scored 0.57844
# clf = nb.SklearnNaiveBayesTextClassifier()  # scored 0.49502
clf.train(train_text_list, train[outVar])

with open('submission_nb.csv', 'wb') as f:
    f.write(idVar + ',' + outVar + '\n')
    for i in range(len(test)):
        row = test[idVar][i] + ',' + ('1' if clf.predict(test_text_list[i]) else '0')
        print(row)
        f.write(row + '\n')